<x-app-layout>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius:12px;border:none;box-shadow:0 8px 32px rgba(0,55,100,.15);">
                <div class="modal-header border-0">
                    <h5 class="modal-title fw-semibold" style="color:var(--hu-azul);">Teléfonos</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body pt-0 text-muted" style="font-size:.9rem;">
                    Aquí se agregan los teléfonos y se asignan a un área o depósito.
                </div>
            </div>
        </div>
    </div>
    ...
    {{-- Modal detalle --}}
    <div class="modal fade" id="infoTelModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content" style="border-radius:12px;border:none;box-shadow:0 8px 32px rgba(0,55,100,.15);">
                <div class="modal-header border-0">
                    <h5 class="modal-title fw-semibold" style="color:var(--hu-azul);">Información del teléfono</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <dl class="row g-2 mb-0" style="font-size:.88rem;">
                        ...
                        <dt class="col-5 text-muted fw-semibold">Número</dt>
                        <dd class="col-7 mb-1" id="numeroInfo"></dd>
    ...
    <div class="modal fade" id="deleteModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content"
                style="border-radius:12px;border:none;box-shadow:0 8px 32px rgba(0,55,100,.15);">
                <div class="modal-header border-0">
                    <h5 class="modal-title fw-semibold" style="color:var(--hu-azul);">¿Eliminar teléfono?</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
                    <form action="{{ route('delete_telefonos') }}" method="POST">
                        @csrf
    ...
    <div class="modal fade" id="historyPcModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content"
                style="border-radius:12px;border:none;box-shadow:0 8px 32px rgba(0,55,100,.15);">
                <div class="modal-header border-0">
                    <h5 class="modal-title fw-semibold" style="color:var(--hu-azul);">Historial del teléfono</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body">
    ...
    <div class="px-4 px-md-5 py-4" style="max-width:1400px;margin:0 auto;">
        <div class="bg-white rounded-3 p-4" style="box-shadow:0 2px 12px rgba(0,55,100,.08);">
            <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
                ...
                <div class="d-flex align-items-center gap-2">
                    <div class="input-group input-group-sm"
                        ...
                        <input type="text" id="search_tel" class="form-control" placeholder="Buscar..."
            ...
            <div id="div-telefono" class="d-flex flex-wrap gap-3 justify-content-start">
                @foreach ($telefonos as $telefono)
                    ...
                    <div class="telefono-card" data-nombre="{{ strtolower($telefono->nombre) }}"
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <span class="material-symbols-outlined"
                                style="font-size:32px;color:var(--hu-azul);">phone</span>
                            <span class="badge"
                        ...
                        <div style="font-size:.78rem;color:var(--hu-texto);"><span class="text-muted">Inv:</span>
                            {{ $telefono->identificador }}</div>
                        <div style="font-size:.78rem;color:var(--hu-texto);"><span class="text-muted">Nº:</span>
                            {{ $telefono->numero ?? '—' }}</div>
                        <div style="font-size:.78rem;color:var(--hu-texto);">
                            @if ($enUso)
                                <span class="text-muted">Área:</span> {{ $telefono->area->nombre ?? '—' }}
                            @else
                                <span class="text-muted">Depósito:</span> {{ $telefono->deposito->nombre ?? '—' }}
                            @endif
                        </div>
                        ...
                        <div class="d-flex gap-1 mt-auto pt-2">
                            <button type="button" class="btn btn-hu btn-sm flex-fill infoBtn" data-bs-toggle="modal"
                                ...
                                data-enuso="{{ $enUso ? 'true' : 'false' }}" data-numero="{{ $telefono->numero }}"
                                data-marca="{{ $telefono->marca_modelo ?? '' }}" title="Ver detalle">
                                <span class="material-symbols-outlined" style="font-size:14px;">info</span>
                            </button>

                            @if (in_array($rolActual, ['Administrador', 'Super administrador', 'Tecnico'], true))
                                <button type="button" class="btn btn-hu btn-sm flex-fill maintenanceBtn"
                                    data-bs-toggle="modal" data-bs-target="#editTelModal"
                                    data-id="{{ $telefono->id }}"
                                    data-identificador="{{ $telefono->identificador }}"
                                    data-nombre="{{ $telefono->nombre }}" data-ip="{{ $telefono->ip }}"
                                    data-area="{{ $telefono->area_id }}"
                                    data-deposito="{{ $telefono->deposito_id }}"
                                    data-enuso="{{ $enUso ? 'true' : 'false' }}"
                                    data-numero="{{ $telefono->numero }}"
                                    data-marca="{{ $telefono->marca_modelo ?? '' }}" title="Editar">
                            ...
                            <button type="button" class="btn btn-hu btn-sm flex-fill historyBtn"
                                ...
                                data-id="{{ $telefono->id }}" data-nro_inv="{{ $telefono->identificador }}"
                                data-nombre="{{ $telefono->nombre }}" data-tipo="Telefono" title="Historial">
                            ...
                            @if ($rolActual === 'Super administrador')
                                <button type="button" class="btn btn-danger btn-sm flex-fill" data-bs-toggle="modal"
                                    data-bs-target="#deleteModal" data-id="{{ $telefono->id }}" title="Eliminar">
                ...
                @if ($telefonos->isEmpty())
                    <div class="text-center text-muted py-5 w-100" style="font-size:.9rem;">
                        <span class="material-symbols-outlined d-block mb-2" style="font-size:2.5rem;">phone</span>
    ...
    @push('scripts')
        <script>
            $(document).ready(function() {
                ...
                // ── Checkbox en uso EDITAR ──
                $('#editEn-uso').on('change', function() {
                    const modal = $('#editTelModal');
                    if ($(this).is(':checked')) {
                ...
                $('#editTelModal').on('show.bs.modal', function(event) {
                    ...
                    $(this).find('#editNumero').val(btn.data('numero'));
                ...
                $('#infoTelModal').on('show.bs.modal', function(event) {
                    ...
                    $(this).find('#numeroInfo').text(btn.data('numero') || '—');
                ...
                $('#search_tel').on('input', function() {
                    ...
                    $('.telefono-card').each(function() {
                        $(this).toggle(String($(this).data('nombre')).includes(term) || String($(this)
                            .data('id')).includes(term));
                    });
                });

                // ── Click en tarjeta ──
                $(document).on('click', '.telefono-card', function(e) {
                    if (!$(e.target).closest('button').length) $(this).find('.infoBtn').trigger('click');
                });
                ...
                $('#historyPcModal').on('show.bs.modal', function(event) {
                    ...
                    $.ajax({
                        ...
                        method: 'GET',
                        success: function(r) {
                            table.rows.add(r.historia.map(h => {
                                const f = new Date(h.created_at).toLocaleDateString(
                                    'es-ES', {
                                        day: '2-digit',
                                        month: '2-digit',
                                        year: 'numeric',
