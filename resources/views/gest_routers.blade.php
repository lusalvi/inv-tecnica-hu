...
<x-app-layout>
                ...
                <div class="modal-body">
                    <form action="{{ route('store_routers') }}" method="POST" id="addRouterForm">
                        ...
                        <div class="p-3 rounded-2 mb-4 d-flex align-items-center gap-2"
                            ...
                            <label class="form-check-label fw-semibold mb-0" for="add-en-uso" style="font-size:.88rem;cursor:pointer;">
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-6" id="add-area-col">
                                ...
                                <select class="form-control @error('addArea') is-invalid @enderror"
                                    id="addArea" name="addArea" disabled>
                                ...
                                <div id="addNroConsul_div" class="mt-2" style="display:none;">
                                    <label class="form-label fw-semibold" style="font-size:.85rem;">Nº consultorio</label>
                        ...
                        <div id="add-detalle-area-div" style="display:none;" class="mb-3">
                            <label class="form-label fw-semibold" style="font-size:.85rem;">Detalle de la ubicación</label>
            ...
            <div class="modal-content" style="border-radius:12px;border:none;box-shadow:0 8px 32px rgba(0,55,100,.15);">
                ...
                <div class="modal-body">
                    <form action="{{ route('edit_routers') }}" method="POST">
                        ...
                        <div class="p-3 rounded-2 mb-4 d-flex align-items-center gap-2"
                            style="background:#EFF4FB;border:1px solid rgba(0,55,100,.15);">
                            <input class="form-check-input m-0" type="checkbox" id="editEn-uso" name="en-uso" style="cursor:pointer;">
                            <label class="form-check-label fw-semibold mb-0" for="editEn-uso" style="font-size:.88rem;cursor:pointer;">
                                En uso (asignado a un área)
                            </label>
                        </div>
                        ...
                        <div class="p-3 rounded-2 mb-3 d-flex align-items-center gap-2"
                            ...
                            <input class="form-check-input m-0" type="checkbox" id="en-uso-mantenimineto" style="cursor:pointer;">
                            <label class="form-check-label fw-semibold mb-0" for="en-uso-mantenimineto" style="font-size:.88rem;cursor:pointer;color:#92400E;">
                                Se realizó mantenimiento
                            </label>
                        </div>
                        <div id="div-detalle-mant">
                            <div class="mb-3" style="display:none;">
                                <label class="form-label fw-semibold" style="font-size:.85rem;">Detalle del mantenimiento</label>
            ...
            <div class="modal-content" style="border-radius:12px;border:none;box-shadow:0 8px 32px rgba(0,55,100,.15);">
                ...
                <div class="modal-body">
                    <table id="table_historias_pc" class="table table-bordered table-striped w-100">
                        <thead>
                            <tr>
                                <th>Técnico</th><th>Detalle</th><th>Motivo</th><th>Fecha</th>
                            </tr>
                        </thead>
        ...
        <div class="bg-white rounded-3 p-4" style="box-shadow:0 2px 12px rgba(0,55,100,.08);">
            <div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
                ...
                <div class="d-flex align-items-center gap-2">
                    <div class="input-group input-group-sm" style="max-width:220px;border:1px solid #ced4da;border-radius:.375rem;">
                        <span class="input-group-text" style="background:#fff;border:0;">
                            <span class="material-symbols-outlined" style="font-size:15px; color:var(--hu-azul);">search</span>
                        </span>
                ...
                @foreach ($routers as $router)
                    ...
                    <div class="router-card" data-nombre="{{ strtolower($router->nombre) }}"
                        ...
                        <div class="d-flex align-items-center justify-content-between">
                            <span class="material-symbols-outlined" style="font-size:32px;color:var(--hu-azul);">router</span>
                            <span class="badge" style="background:{{ $enUso ? '#D1FAE5' : '#FEF3C7' }};color:{{ $enUso ? '#065F46' : '#92400E' }};font-size:.7rem;font-weight:600;border-radius:6px;">
                                {{ $enUso ? 'En uso' : 'Depósito' }}
                            </span>
                        </div>

                        <div class="fw-semibold" style="font-size:.9rem;color:var(--hu-azul);line-height:1.2;">{{ $router->nombre }}</div>
                        ...
                        <div class="d-flex gap-1 mt-auto pt-2">
                            <button class="btn btn-hu btn-sm flex-fill infoBtn" data-bs-toggle="modal"
                                data-bs-target="#infoRouterModal"
                                data-id="{{ $router->id }}"
                                data-identificador="{{ $router->identificador }}"
                                data-nombre="{{ $router->nombre }}"
                                data-ip="{{ $router->ip }}"
                            ...
                            @if (Auth::user()->rol->nombre == 'Administrador' || Auth::user()->rol->nombre == 'Super administrador' || Auth::user()->rol->nombre == 'Tecnico')
                                <button class="btn btn-hu btn-sm flex-fill maintenanceBtn" data-bs-toggle="modal"
                                    ...
                                    data-nombre="{{ $router->nombre }}"
                                    data-ip="{{ $router->ip }}"
                                    data-area="{{ $router->area_id }}"
                                    data-deposito="{{ $router->deposito_id }}"
                                    data-enuso="{{ $enUso ? 'true' : 'false' }}"
                                    data-marca="{{ $router->marca_modelo ?? '' }}"
                                    data-area_detalle="{{ $router->area_detalle ?? '' }}"
                                    title="Editar">
    ...
    $(document).ready(function () {
        ...
        $('.maintenanceBtn, .historyBtn').on('click', function (e) { e.stopPropagation(); });
            ...
            $.ajax({ url, method: 'GET',
                success: function (response) {
                    const data = response.historia.map(h => {
                        ...
                        return [`<b>${h.tecnico}</b>`, `<b>${h.detalle}</b>`, `<b>${h.motivo||''}</b>`, `<b>${fecha}</b>`];
                    });
        ...
        $('#table_historias_pc').DataTable({
            dom: 'Bfrtip',
            buttons: [{ extend: 'excelHtml5', text: 'Exportar a Excel',
                className: 'btn btn-hu btn-sm', title: () => 'Historia de ' + nro_inv + ' - ' + nombre }],
